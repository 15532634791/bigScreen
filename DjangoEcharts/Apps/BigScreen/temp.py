...
class ParseExcelWriteToDB(object):
    ...
    def update_screen_model(self):
        # 获取InstallMaintainScore模型的最新评分子查询
        install_records = InstallMaintainScore.objects.all().values_list('userid', 'score')
        for record in install_records:
            userid, score = record  # 解包元组
            # 处理 userid 和 score 的值
            # 更新BigScreen模型的scoring_by_install_and_maintain_services字段
            BigScreen.objects.filter(userid=userid).update(scoring_by_install_and_maintain_services=score)

        internet_records = InternetQualityScore.objects.all().values_list('userid', 'score')
        for record in internet_records:
            userid, score = record  # 解包元组
            # 处理 userid 和 score 的值
            # 更新BigScreen模型的internet_quality_rating字段
            BigScreen.objects.filter(userid=userid).update(internet_quality_rating=score)

    def update_avg_score(self):

        all_userid = BigScreen.objects.all().values('userid')
        # print(all_userid)
        for queryset in all_userid:
            userid = queryset.get('userid')

            install_records = InstallMaintainScore.objects.filter(userid=userid).values('score')
            internet_records = InternetQualityScore.objects.filter(userid=userid).values('score')
            try:
                install_score = int(install_records[0].get('score')) if install_records else None
                internet_score = int(internet_records[0].get('score')) if internet_records else None
            except Exception as e:
                install_score = 0
                internet_score = 0
            ...
            print(install_score, internet_score, comprehensive_score)
            BigScreen.objects.filter(userid=userid).update(comprehensive_score=comprehensive_score)
    ...
    def init_data(self, workbook: object, worksheet: object, model_object: object):
        # 获取特定的表单
        ...
        handle_data = self.distinct_max(data_list)

        write_to_list = []
        for key, value in handle_data.items():
            print(key, value)
            write_to_list.append(model_object(
                userid=key,
                score=value
            ))
        model_object.objects.bulk_create(write_to_list)

    def update_or_create_score(self, workbook: object, worksheet: object, model_object: object):
        # 获取特定的表单
        ws = workbook[worksheet]  # 或者 wb['Sheet1'] 如果你知道表单名字
        # 遍历表单的行
        for number, row in enumerate(ws.iter_rows(values_only=True), start=1):
            ...
            if number != 1:
                try:
                    model_object.objects.update_or_create(
                        userid=row[0],
                        score=row[1]
                    )
                except Exception as e:
    ...
    def parse_excel(self):
        ...
        if "第一次初始化用户打分数据" in filename:
            for worksheet in sheet_names:

                if worksheet == '装维服务打分':
                    self.init_data(workbook=wb, worksheet=worksheet, model_object=InstallMaintainScore)
                elif worksheet == '上网质量打分':
                    self.init_data(workbook=wb, worksheet=worksheet, model_object=InternetQualityScore)
        ...
        if len(sheet_names) > 1:
            for worksheet in sheet_names:
                ...
                if worksheet == '装维服务打分':
                    self.update_or_create_score(workbook=wb, worksheet=worksheet, model_object=InstallMaintainScore)
                elif worksheet == '上网质量打分':
                    self.update_or_create_score(workbook=wb, worksheet=worksheet, model_object=InternetQualityScore)
        else:
            # 获取特定的表单
            self.write_to_big_screen(workbook=wb, worksheet=sheet_names[0])
...
class UploadFileViewSet(viewsets.ModelViewSet, ParseExcelWriteToDB):
    ...
    def perform_create(self, serializer):
        ...
        thread.start()
        # self.parse_excel()
        thread = threading.Thread(target=self.update_avg_score)
        thread.start()
